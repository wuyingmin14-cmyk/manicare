...
1. **多模态 LLM 直接推荐**: qwen-vl-plus 一次完成款式推荐 + 指甲定位
2. **MediaPipe 3D 关键点驱动曲率感知**: z 坐标 + 关节角 → 真实感 shading
3. **像素级 GrabCut 分割**: 椭圆初始化 + 凸包填补 + 肤色 hint
4. **流程**: 选款式 → 上传 → 试戴 → Chatbot 咨询 (4 步, 一次完成)
5. **承认局限**: "假 3D 渲染" → "真 3D 重建" 作为未来工作
